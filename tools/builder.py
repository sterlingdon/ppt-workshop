...
async def extract_layout_and_assets():
    # 确保输出目录存在
    os.makedirs("output/pro_assets", exist_ok=True)
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.goto("http://localhost:5173/?extract=1")
            ...
            for slide_index, slide_loc in enumerate(slide_locators):
                ...
                slide_info = {
                    ...
                    "texts": [],
                    "components": [],
                    "bg_path": f"output/pro_assets/slide_{slide_index}_bg.png"
                }
                ...
                for bg_index, bg_loc in enumerate(bg_locators):
                    ...
                    comp_path = f"output/pro_assets/slide_{slide_index}_comp_{bg_index}.png"
            ...
            with open("output/layout_manifest.json", "w", encoding="utf-8") as f:
                json.dump({"slides": slides_data}, f, indent=4, ensure_ascii=False)
            
            print(">>> [Phase 3] 提取成功，布局清单写入 output/layout_manifest.json")

    finally:
        vite_process.terminate()

if __name__ == "__main__":
    asyncio.run(extract_layout_and_assets())
